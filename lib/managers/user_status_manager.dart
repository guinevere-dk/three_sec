...
class UserStatusManager {
  ...
  Future<void> initialize() async {
    try {
      ...
      if (nextTierEffectiveAtMillis != null) {
        _nextTierEffectiveAt = DateTime.fromMillisecondsSinceEpoch(
      ...
      print(
        ...
        'tier=$_currentTier, productId=$_productId, userId=${_maskUid(_userId)}, '
  ...
  bool isStandardOrAbove() {
    return _currentTier == UserTier.standard ||
  ...
  Future<bool> evaluateAndAutoDowngradeIfExpired({
    ...
    try {
      if (_currentTier == UserTier.free) {
        print(
          '[UserStatusManager][Expiry] skip: already free (reason=$reason)',
        );
        return false;
      }
      ...
      final shouldDowngrade =
          nowKst.isAfter(boundaryKst) || nowKst.isAtSameMomentAs(boundaryKst);
      ...
      final downgraded =
          await downgradeExpiredSubscriptionToFreePreservingHistory(
            reason: reason,
          );
      if (downgraded) {
  ...
  Future<bool> downgradeExpiredSubscriptionToFreePreservingHistory({
    ...
    try {
      ...
      print(
        '[UserStatusManager][Expiry] expired paid tier downgraded with purchase history preserved '
        '(reason=$reason)',
      );
      return true;
    } catch (e) {
  ...
  Future<bool> setUserId(String uid) async {
    try {
      ...
      _userId = uid;
      print('[UserStatusManager] 사용자 ID 저장: ${_maskUid(uid)}');
      return true;
    } catch (e) {
