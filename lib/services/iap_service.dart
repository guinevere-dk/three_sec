...
class IAPService {
  ...
  static const String premiumLifetime = '3s_premium_lifetime';
  ...
  Future<bool> _loadProducts() async {
    try {
      ...
      for (var product in _products) {
        print('[IAPService] 상품: ${product.id} | ${product.title} | ${product.price}');
      }
  ...
  Future<void> _onPurchaseUpdate(List<PurchaseDetails> purchaseDetailsList) async {
    for (final PurchaseDetails purchase in purchaseDetailsList) {
      print('[IAPService] 구매 상태 업데이트: ${purchase.productID} → ${purchase.status}');
  ...
  Future<void> _deliverProduct(PurchaseDetails purchase) async {
    try {
      ...
      // 상품 ID에 따라 등급 결정
      UserTier tier;
      if (productId == standardMonthly) {
        tier = UserTier.standard;
      } else if (productId == premiumMonthly || productId == premiumLifetime) {
